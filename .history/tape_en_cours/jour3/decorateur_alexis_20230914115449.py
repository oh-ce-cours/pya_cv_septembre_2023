# Décorateur pour les ingrédients
def ingredient_haut(nom_ingredient):
    def decorator(f):
        def wrapper(*args, **kwargs):
            print(f"{nom_ingredient}") 
            sortie = f(*args, **kwargs)
            return sortie
        return wrapper
    return decorator


def ingredient_bas(param):
    def decorator(f):
        def wrapper(*args, **kwargs):
            sortie = f(*args, **kwargs)
            print(f"{param}") 
            return sortie
        return wrapper
    return decorator

# Décorateur pour le pain
def decorateur_generique(param):
    def decorateur_pain(f):
        def wrapper(*args, **kwargs):
            print("Pain")
            result = f(*args, **kwargs)
            print("Pain")
            return result
        return wrapper
    return decorateur_pain

# Fonction pour créer un sandwich
@pain
@ingredient_haut("salade")
@ingredient_haut("tomate")
@ingredient_bas("salade")
@ingredient_bas("fromage")
def sandwich(viande):
    print(viande)

sandwich = ingredient_bas("salade")(ingredient_bas("fromage")(sandwich))

# Créez un sandwich en utilisant le décorateur
sandwich("steak")